...
protected:
    /** 本身这个类也是一个生产者，每次客户端读出一个packet去我们
     * 就会把伴奏的packet给放入队列中，把原唱的packet销毁掉 **/
    ...
    virtual void decoderSongPacket();
...
public:
    ...
    virtual int
    getMusicMeta(const char *accompanyPath, const char *originalSongPath, int *accompanyMetaData);

    /** 初始两个decoder,并且根据上一步算出的采样率，计算出伴奏和原唱的bufferSize **/
    virtual void
    init(const char *accompanyPath, const char *originalSongPath, float packetBufferTimePercent);
